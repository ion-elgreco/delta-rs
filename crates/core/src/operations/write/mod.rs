...
use datafusion_proto::protobuf::scalar_nested_value;
...
use datafusion_common::{DFSchema, Result as DataFusionResult, ToDFSchema};
use datafusion_expr::{col, lit, when, Expr, ExprSchemable, Extension, LogicalPlan, LogicalPlanBuilder, UserDefinedLogicalNode};
use execution::{
    prepare_predicate_actions, write_execution_plan_v2,
};
...
use crate::kernel::{Action, ActionType, DataCheck, Metadata, StructType, StructTypeExt};
            ...
            if let Some(snapshot) = &this.snapshot {
                if matches!(this.mode, SaveMode::Overwrite) {
                    ...
                    match predicate.as_ref() {
                        Some(pred) => {
                            let (predicate_actions, scan_df) = prepare_predicate_actions(
                                pred.clone(),
                            ...
                            if let Some(scan_df) = scan_df {
                                let negated_expression =
                                ...
                                if should_write_cdc(snapshot)? {
                                    ...
                                    input_df =
                                        input_df.union(scan_df.filter(negated_expression)?)?;
                                }
                            }
